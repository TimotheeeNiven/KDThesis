...
# Parameters
batch_size = 64
input_size = 224  # ViT-L/16 requires 224x224 input
epochs = 20
ft_epochs = 10
fine_tune = True
...
# Load ViT-L/16 model
print("Loading pre-trained ViT-L/16 model...")
model = create_model("vit_large_patch16_224", pretrained=True)
in_features = model.head.in_features
model.head = nn.Linear(in_features, 100)  # Adjust for CIFAR-100
model.to(device)
...
torch.save(model.state_dict(), "vit_l16_cifar_top.pth")
...
if fine_tune:
    ...
    torch.save(model.state_dict(), "vit_l16_cifar_distilled.pth")
